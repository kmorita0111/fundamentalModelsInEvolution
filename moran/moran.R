...
for ( num_trial in 1:all_trial ) {
  ...
  for ( t in 1:geneNum ) {
    ...
    #print( prob_weight )
    ind_num <- sample( 1:Nall, 1 ) # an extinct parent
    #print( paste( "extinction no.", as.character(ind_num), sep = "" ) )
    newInd <- sample( num, size=1, replace=TRUE, prob=prob_weight )
    numAfter[ind_num] <- newInd
    #print( paste( "child's allele is", as.character(newInd) ) )
    num <- numAfter
